...
for id in ${hosts//,/ }; do
  ...
  while IFS=$'\t' read -r type name relative rest; do
    ...
    printf '\n\n## Installation binding\nBinary: `%s/midden` (quote its path). Prefer available Midden workflow MCP tools. CLI discovery uses agent list and agent schema <capability>; execute agent <capability> --input - --home `%s`. Read _view handles with results.inspect rather than formatting large JSON in shell.\nPreserve read-only source stores. Your CLI owns models and permissions. Author requested local drafts here with recipes.compose; they remain unreviewed. Positive host confirmation is required for review/export, not reversible local drafting.\n' "$install_dir" "$state_dir" >> "$source"
