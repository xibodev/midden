...
Midden is a local binary. Every step is one command; nothing needs a file
written first.
...
`midden ls` reports `total` after filtering and `matched` before it, so the
larger number is `matched`. It also names any source store it could not read;
when that happens the counts cover only the stores that opened.

`midden assay` is free and runs no model. It reports size, signal share and
the bounded candidate set. `slice_bytes` is what governs cost, not
`signal_bytes`.

If a command fails, read what it says rather than guessing: Midden
distinguishes "no sessions matched" from "the source stores could not be
seen at all", and those need different answers.
