...
try {
    ...
    foreach ($id in $Hosts) {
        foreach ($skill in $skills) {
            ...
            $content+="`n`n## Installation binding`nBinary: ``$binary``. Invoke with PowerShell's & operator and a quoted path. Prefer available Midden workflow MCP tools. CLI discovery uses agent list and agent schema <capability>; execute agent <capability> --input - --home ``$StateDir``. Read _view handles with results.inspect rather than formatting large JSON in shell.`nThe advanced module interface uses roots.midden_home.path ``$StateDir`` and mode rw. Preserve read-only source stores. Your CLI owns models and permissions. Author requested local drafts here with recipes.compose; they remain unreviewed. Positive host confirmation is required for review/export, not reversible local drafting.`n"
